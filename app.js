...
function renderProducts(){const q=($('#searchInput').value||'').toLowerCase();const cat=filter.value;const list=products.filter(p=>(cat==='all'||p.category===cat)&&(p.name.toLowerCase().includes(q)||p.category.toLowerCase().includes(q)));productGrid.innerHTML=list.map(p=>`<div class="product-card"><div class="prod-img">${p.img||'🧶'}</div><h3>${p.name}</h3><p>${p.desc||p.category}</p><div class="price">${money(p.price)}</div><button class="add-btn" onclick="addToCart(${p.id})">Add to Cart</button></div>`).join('')||'<p>No products found.</p>'}
function addToCart(id){const p=products.find(x=>x.id===id);const item=cart.find(x=>x.id===id);item?item.qty++:cart.push({...p,qty:1});save();renderCart();toast('Added to cart')}
...
window.addToCart=addToCart;window.removeCart=removeCart;
